...
def promptsFile(folderPathway):
    # Function promptsFile
    # Converts all ", prompts" files into a single document
    # Inputs  : folderPath - Pathway for the folder
    # Outputs : NONE

    wholeList = (os.listdir(folderPathway))  # Call entire folder contents
    mainFileList = []  # Initialize list variable
    for doc in wholeList:  # Repeat with each item
        if doc[-4:] == ".txt":  # If item is .txt
            if doc[-12:-4] == ", prompt":  # If item is Prompt
                mainFileList.append(doc)  # Add good item to list

    promptFile = []  # New file text
    for fileName in mainFileList:  # For eligable files add text to file
        filePath = folderPathway + chr(92) + fileName
        newName = fileName.replace(", prompt.txt", "")  # Erase file type
        promptFile.append(newName + ":")  # Add og title
        with open(filePath) as file:  # Open the text file
    ...
    newDoc = open(folderPathway + chr(92) + "Prompts.txt", "w")
    # print(promptFile)
    fileTxt = '\n'.join(promptFile)
    # print(fileTxt)
    newDoc.write(fileTxt)
...
folderPathway = r"C:\Users\zaper\Downloads\OG PAGES DOCS\Found, G , F"  # Declare main folder pathway
