...
class primeWindow(QMainWindow):  # Creates primary window object
    ...
    def __init__(self):  # Names the figure window as "self"
        ...
        self.letterInfo = centeredLabel(
            "Enter - Punch \n  I - Info\n N - New", 20, self
        )  # Explain key press values
    ...
    def punchEntered(self, pin: str):
        ...
        if pin == "9999":  # If admin
            ...
            currTime = datetime.fromtimestamp(round(time.time())).strftime(
                "%A, %B %d, %Y %I:%M:%S"
            )  # Format time
...
class infoWindow(QMainWindow):  # Creates information window object
    ...
    def createBoxes(self):
        ...
        createDate = datetime.fromtimestamp(self.userInfo["creationDate"]).strftime(
            "%A, %B %d, %Y %I:%M:%S"
        )  # Format create date
        self.basicDataLabel("Creation Date", createDate)  # Generate create date box

        infoWidget.setGeometry(
            50, 50, int(self.winSize[0] * 0.6), self.winSize[1] - 200
        )  # Set the size and position of the central widget relative to the screen
    ...
    def basicDataLabel(self, key, value, edit=0):
        ...
        self.valueBoxes.append(
            textBox
        )  # Append the text box widget to the list for later reference
    ...
    def savePin(self):
        ...
        keys = [
            ...
            "E-Mail",
            "Address",
        ]  # List of keys to save values from
    ...
    def onPassPress(self):
        ...
        if self.passWord.text() == "":  # If it is the password
            ...
            centralWidget.setGeometry(
                int(self.winSize[0] * 0.4), 50, int(self.winSize[0] * 0.6), self.winSize[1] - 200
            )  # Set the size and position of the central widget
            centralWidget.show()
